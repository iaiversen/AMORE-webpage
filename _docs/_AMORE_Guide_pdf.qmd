...
-   **Information container** - The top blue title panel with project details, status badge, authors, project info, identifiers, and contact

-   **Abstract** - Brief overview of the meta-analysis

-   **Version History** - Expand/collapse timeline of all versions with resource links (replaces the old tabbed Resources section)

#### **Optional sections** (include if relevant):

-   Current Results

-   Methodology (parent heading `##`) containing subsections (`###`):

    -   Inclusion and Exclusion Criteria

    -   Search Strategy

    -   Related Research (for previous meta-analyses this project builds on)
...
status: "living" # "living" or "retired" — controls the status badge in the hero section
update-cadence: "24 months" # e.g. "6 months", "12 months", "24 months" — shown in the hero status badge
next-update: "Month YYYY" # only for living status — shown in hero and Project info card
final-version: "" # only for retired status, e.g. "v3 (2028)" — shown when status is "retired"
...
<!-- Living status badge (use this for active projects) -->
<div class="project-status project-status--living">
  <span class="status-dot status-dot--living"></span>
...
<!-- Retired status badge (use this instead for completed projects)
<div class="project-status project-status--retired">
  <span class="status-dot status-dot--retired"></span>
...
<!-- NOTE: The Authors card should list ALL authors across ALL versions of the project. -->
...
The Version History section replaces the old tabbed Resources interface. It uses an expand/collapse pattern where each version of the project is a collapsible entry showing publication date, version-specific authors, and resource links as compact pill-buttons.

**Key design principles:**

-   The **newest version is expanded by default**, older versions are collapsed

-   Each collapsed version shows an **"N resources" count** as summary

-   When there are **4+ versions**, a "Show N earlier versions" toggle hides the oldest ones

-   **Related studies and previous meta-analyses do NOT belong here** — they go in the optional Related Research subsection under Methodology

-   **Version authors** in each entry should list only the authors for that specific version, while the Authors info card shows ALL authors across ALL versions

#### Version Entry Structure

Each version entry follows this HTML structure. Copy and paste for each new version, updating the version number, label, date, authors, and resource links:
  ...
  <div class="version-entry version-latest" data-version="1">
    ...
    <div class="version-body" id="version-1-body">
      ...
      <div class="version-authors">
        ...
        <!-- EDIT: List only authors for THIS version -->
      </div>

      <div class="version-resources-grid">
        <!-- EDIT: Add one link per resource. Copy the <a> block for each. -->
        <a href="[URL]" class="version-resource-link" target="_blank" rel="noopener">
          <svg class="version-resource-icon" width="16" height="16" viewBox="0 0 24 24"
...
-   **MUST EDIT:** Content within `[brackets]` — dates, author names, URLs, resource labels, resource counts

-   **CAN EDIT:** The `<span>` text inside each resource link (e.g., "Publication", "Pre-registration", "Dataset", "Shiny App", "Preprint")

-   **DO NOT EDIT:** HTML tags, class names, SVG elements, or `data-` attributes

#### Adding a New Version

When a new update is published:

1.  Move the `version-latest` class from the previous newest entry to the new one

2.  Set `aria-expanded="false"` and add `hidden` to the body of the old version

3.  Add the new version entry above the old one with `version-latest` class and `aria-expanded="true"`

**Example: Adding Update 1 (v2) above Initial Publication (v1):**

``` html
<div class="version-history">

  <!-- v2 is now the latest -->
  <div class="version-entry version-latest" data-version="2">
    <button class="version-header" aria-expanded="true" aria-controls="version-2-body">
      ...v2 content...
    </button>
    <div class="version-body" id="version-2-body">
      ...v2 resources...
    </div>
  </div>

  <!-- v1 is now collapsed (no longer version-latest) -->
  <div class="version-entry" data-version="1">
    <button class="version-header" aria-expanded="false" aria-controls="version-1-body">
      ...v1 content...
    </button>
    <div class="version-body" id="version-1-body" hidden>
      ...v1 resources...
    </div>
  </div>
...
#### Show Earlier Versions Toggle (4+ versions)

When there are 4 or more versions, hide the oldest versions behind a toggle. Uncomment this block and place older version entries inside the `version-earlier-group`:
...
<div class="version-earlier-group" hidden>
  <!-- Move older version-entry elements here -->
</div>
```

#### JavaScript (DO NOT EDIT)

The following script handles expand/collapse and the "Show earlier versions" toggle. It is included once at the end of the Version History HTML block:
...
### Methodology (parent section)

All methodology-related subsections are now grouped under a single `## Methodology` heading. This ensures the Quarto native Table of Contents (TOC) works correctly with subsections appearing as nested items. The TOC structure will be:
...
**Important:** Use `##` for the Methodology heading and `###` for all subsections within it. Use `####` for sub-sub-sections like "Inclusion Criteria" and "Exclusion Criteria".
...
#| description: Nested under ## Methodology as ### subsection. Use #### for Inclusion/Exclusion sub-headers.
#| eval: false
#| echo: true
...
Search strings should be wrapped in backticks for proper formatting of special characters (quotes, parentheses, Boolean operators, registered symbols, etc.).

``` r
{r}
#| label: Search strategy of main content area
#| description: Write the search strings used on different search engines. Wrap search strings in backticks for proper formatting.
#| eval: false
#| echo: true
...
::: search-string-box
[EMBASE:]{.search-db} `'Pitocin®' OR 'Syntocinon®' OR 'synthetic oxytocin' AND 'labour/labor' OR 'birth'`

[Web of Science:]{.search-db} `('Pitocin®' OR 'Syntocinon®' OR 'synthetic oxytocin') AND ('delivery' AND 'augment*' OR 'induc*')`

[MEDLINE:]{.search-db} `'Pitocin®' OR 'Syntocinon®' OR 'synthetic oxytocin' AND 'labour/labor' OR 'birth' AND 'antepartum'`
:::
```

**Note:** The `[Database:]{.search-db}` syntax creates a styled label. The backtick-wrapped search strings preserve special characters and formatting.

### Related Research (optional)

Include this subsection if the project builds on previous meta-analyses or key foundational studies. This is separate from the Version History — related studies that inspired the project belong here, not in version entries.

``` r
{r}
#| label: Related Research
#| description: Optional subsection under Methodology. Include previous meta-analyses the project builds on.
#| eval: false
#| echo: true
...
This living meta-analysis builds on [Previous Author et al. (YYYY)], which concluded [key finding].

- [Previous meta-analysis title](https://doi.org/XXXX)
- [Related foundational study](https://doi.org/XXXX)
```
...
#| description: Now a ### subsection under ## Methodology.
#| eval: false
#| echo: true
