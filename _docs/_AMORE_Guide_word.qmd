...
See the `_lma-template.qmd` file and the PDF version of this guide for the complete HTML code structure and examples of how to add new versions.
...
### Methodology (parent heading)

**Important:** All methodology-related subsections are now grouped under a single `## Methodology` parent heading. This ensures proper Quarto TOC navigation. The subsections below (Inclusion/Exclusion, Methodology Summary, Search Strategy, Related Research) all use `###` headings and sit under this parent `##` heading.
...
**Note:** This is now a `###` subsection under `## Methodology`. The criteria boxes use `####` headings.
...
**Note:** This is now a `###` subsection under `## Methodology` (not a standalone `##` heading).
...
**Note:** This is now a `###` subsection under `## Methodology`. Search strings should be wrapped in backticks for consistent styling across all projects.

``` r
{r}
#| label: Search strategy of main content area
#| description: Write the search strings used on different search engines like PubMed or similar for reproducibility.
#| eval: false
#| echo: true
...
*Note: For full search strategy details, see the project's pre-registration.*
```

### Related Research (optional)

This is an optional `###` subsection under `## Methodology` for listing previous meta-analyses or related studies that inspired the project. **Related studies do NOT belong in the Version History** — they go here instead.

``` markdown
### Related Research

- [Author et al. (Year). Title of related study.](https://doi.org/...)
- [Author et al. (Year). Title of another related study.](https://doi.org/...)
```
