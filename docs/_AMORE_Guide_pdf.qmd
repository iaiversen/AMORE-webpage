...
two-step authenticator connected to
...
If you need editorial right to the nettskjema, reach out to Dan or Ingebjørg.
...
### Code block types throughout this guide and what they mean in the document

Throughout this guide, you'll see code blocks with different labels that indicate language used, or where/how to run the code.
...
The bash code block indiactes that the content is for a command line interface. Bash, powershell and RStudio Terminal are all command line interfaces. (I personally use Windows Powersehll as my terminal).

The blocks contains git commands. Git commands work in Bash, Powershell and RStudio Terminal.

Shift+Alt+T or Shift+Option+M on mac opens Terminal in RStudio.

#### {html} and {scss} and {yaml} blocks 

Html and scss code blocks indicates that the content in the code block is written using the specified language. The languages are used across different files and scripts all withing the AMORE.Rproj project. Scss is mainly found in the styles.scss script but is also used in other scripts.

#### {yaml} - yaml blocks 

YAML is a format for storing structured information, like filling out a form with specific fields. In AMORE, the YAML header at the top of each project file contains all the searchable metadata that allows the Shiny app to find, filter, and display projects correctly. Yaml code is always wrapped, and the code blocks contains the information that would be inside the yaml formation.
...
-   **See `{bash}`** → Open Terminal (Git Bash, Windows powershell, or RStudio Terminal)
...
![](images/Screenshot 2025-10-30 133047.png){width="414"}

------------------------------------------------------------------------

# Troubleshooting

This section proposes different solutions to common errors and problems you mey encounter when working on AMORE-website.
...
Ctrl+Shift+Z undo changes, and is easy to use for smaller changes you want to undo.
...
![Inspect element in browser](images/Screenshot 2025-10-30 134418.png){fig-align="center"}
...
![](images/Screenshot 2025-10-30 140616.png)
...
``` r
#| label: quarterly-package-updates
#| description: Update R packages to latest stable versions
#| eval: false
#| echo: true
...
# Update specific packages (test individually first)
update.packages(c("quarto", "rmarkdown", "shiny", "rsconnect"), 
                ask = FALSE, 
                checkBuilt = TRUE)

# After updating, test critical functionality:
# 1. Render a project page: quarto render LMAs/example.qmd
# 2. Test Shiny app locally: shiny::runApp("shiny-meta/app.R")
# 3. If tests pass, commit and push updates
```
...
1.  **Domain management** - How to manage amore-project.org

2.  **Access management** - How to add/remove collaborators
